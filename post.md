...
above. In `strip_punctuation_from_name()`, the unpunctuated name is generated by
calling `delete()` on the original name. In `capitalize_names()`, the capitalized
name is generated by calling `title()` on the original name. If `replace()` and
title() are not functional, strip_punctuation_from_name() and
